...
public class TerrainBuilder implements Builder {
    ...
    public void buildGrass(int number) {
        for (int i = 0; i < number; i++) {
            Tile grass = new TileGrass(x * PARTSIZE, y * PARTSIZE, "texture\\grass\\" + grassTextures.get(new Random().nextInt(grassTextures.size())));
            terrain[x++][y] = grass;
            if (x == width) {
                x = 0;
                y++;
    ...
    public void buildWater(int number) {
        for (int i = 0; i < number; i++) {
            Tile water = new TileWater(x * PARTSIZE, y * PARTSIZE, "texture\\water\\" + waterTextures.get(new Random().nextInt(waterTextures.size())));
            terrain[x++][y] = water;
            if (x == width) {
                x = 0;
                y++;
